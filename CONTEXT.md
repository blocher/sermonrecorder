...
**Study artifact**:
One independently editable piece of derived content on a Sermon (short summary, long summary, outline, Quotations, Practical next steps, One next action, adult discussion questions, kids discussion questions, Sermon feedback, Related sources, Doctrinal review, Hymn, compatible hymn tunes, Quiz, and similar). Quotations contain one to three impactful excerpts copied word-for-word from the Transcript. Practical next steps are a short list of grounded ways to live differently; One next action is a single brief, concrete call to action. Sermon feedback is constructive craft critique for conveying the message more clearly — not a Congregant's private Reflection and not a doctrinal audit. Related sources are specific magisterial or scholarly documents suggested for further study (via Magisterium AI Search). Doctrinal review flags sermon assertions that may be heretical or borderline relative to Catholic teaching, with cited magisterial or scholarly documents. A Hymn is original, sermon-inspired verse written in a named meter; compatible hymn tunes are familiar Anglican, Methodist, or Catholic tunes in that same meter. A Quiz pairs comprehension questions with Transcript-grounded answers. Regenerating one artifact does not replace the others; Congregant edits to an artifact are kept until that artifact is regenerated. Key themes are not a separate artifact — they surface as suggested Tags.
_Avoid_: Generated content (as a single blob), AI output, section

**Related source**:
A magisterial or scholarly document suggested for further study of a Sermon's themes, grounded via Magisterium AI Search rather than invented citations. Distinct from Related Sermon (same-library tag similarity).
_Avoid_: Citation (too broad — Doctrinal review also cites), bibliography entry (implementation)

**Doctrinal review**:
A Study artifact that lists specific sermon assertions that may be heretical or borderline relative to Catholic Church teaching, each with an explanation and cited magisterial or scholarly sources. Advisory only; the Congregant verifies against the Church. Lives on the Feedback tab with Sermon feedback.
_Avoid_: Heresy detector (hostile tone), orthodoxy score, Magisterium check (vague)
