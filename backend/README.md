...
The default provider pipeline isolates pew audio with ElevenLabs Voice Isolator, loudness-normalizes the stored playback file, then runs Speechmatics enhanced batch transcription with speaker diarization. Intentional-service cleanup and Study artifacts still use OpenAI via simpleai (`OPENAI_API_KEY`). Set `ELEVENLABS_API_KEY` and `SPEECHMATICS_API_KEY` before running workers. To fall back to the legacy OpenAI diarized transcriber, set `SERMON_TRANSCRIBER=sermons.openai_transcriber.OpenAIDiarizedTranscriber`. Audio over 24 MB is still chunked with `ffmpeg` only on that legacy path; install ffmpeg in every worker image.
